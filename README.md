...
> журнал, шаблоны значений, избранное и созданные вами типы ПЧ — лежит в папке
> **`data` рядом с программой**: куда установили, туда и данные. Программу можно
> поставить в любой каталог, она сама поймёт, где искать и куда писать, а всю
> папку целиком можно скопировать на другой компьютер или на флешку.
>
> Точный путь всегда виден в меню **Справка → О программе**, а сама папка
> открывается через **Справка → Открыть папку с данными**.
>
> Импортированный проект **копируется** туда же — путь, откуда его взяли, нигде
> не запоминается. Созданные и импортированные проекты всегда лежат вместе.
>
> Если писать рядом с программой нельзя (установка в `Program Files` без прав),
> приложение откатывается на `Документы\Modbus Controller`, а затем на
> `%APPDATA%\modbus-controller`. Данные из прежних мест переносятся при первом
> запуске — поштучно, то есть проекты из разных мест собираются вместе, а
> существующие не перезаписываются.
>
> Обновление данные не трогает, хотя обычно папка установки при этом стирается:
> деинсталлятору отдельно объяснено, что `data` удалять нельзя
...
Значок берётся из логотипа `build/logo-source.png` и пересобирается командой:

```bash
node build/make-icon.cjs --from-source   # из логотипа (обычный путь)
node build/make-icon.cjs                 # запасной: нарисовать знак с нуля
```

Результат — `build/icon.png` (1024×1024, прозрачный фон), из него
`electron-builder` сам делает `.ico` для Windows и `.icns` для macOS.

Просто «стереть цвет фона» в этом логотипе нельзя: в нём всего два цвета, и
буквы не залиты — они лишь обведены линиями, причём обводка разомкнута. Поэтому
скрипт разбирает рисунок геометрически: у большой `F` контур собирается из
найденных штрихов (длинные горизонтали — низ перекладин, вертикали — правые
края, крайние точки краски — левый и верхний край буквы, которые в исходнике
совпадают с обрезом холста), а подпись `FBEST` нарисована штриховым шрифтом,
где линии и есть сами буквы — их штрихи перекрашиваются в синий, иначе белым
на прозрачном фоне подпись не видна на светлом рабочем столе.
