...
- `monitor:start { deviceId, paramIds? }` / `monitor:stop { deviceId? }` — раз в секунду читает параметры (по умолчанию группа `F0`, либо явный список `paramIds`), шлёт `monitor:data { deviceId, data }`. Мониторинг нескольких устройств одновременно поддерживается — интервалы хранятся в `Map<deviceId, interval>` (`monitorIntervals`), а не в одном общем таймере; `monitor:stop` без `deviceId` останавливает все сразу (используется при разрыве соединения/скане шины/смене проекта). Все запросы всё равно идут через общий мьютекс `ModbusService.withLock()`, так как это одна физическая шина — при мониторинге многих устройств параллельно обновление каждой колонки будет отставать от номинальной секунды пропорционально общему числу опрашиваемых параметров.
