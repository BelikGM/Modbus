...
- **`Monitor.jsx`** — реалтайм-мониторинг одного устройства: карточки со значениями (drag-n-drop порядок через `@dnd-kit`, графики через `recharts`, экспорт в CSV), параметры и их порядок — из `monitorParams.js`; параметры типа `bitmask` показываются не графиком, а тегами по каждому биту. Пороговые **оповещения** (`device.alerts`, см. ниже). Подсветка ошибки для параметра `F0.10` (жёстко зашитый id — актуально только для моделей Pump; у VL код ошибки лежит в параметре `FAULT_CODE`/`D0.45`, эта авто-подсветка на них не распространяется)
