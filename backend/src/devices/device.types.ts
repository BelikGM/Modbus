...
export interface DeviceConfig {
  ...
  // Что из моделей/прошивок пришло из ПОСТАВКИ (эти строки удалять нельзя —
  // дополнения только добавляются поверх). Заполняется при выдаче.
  builtinModels?: { code: string; powerKw: number; supply?: string }[];
  builtinFirmwares?: string[];
