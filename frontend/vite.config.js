...
export default defineConfig({
  plugins: [react()],
  build: {
    // Каталог сборки лежит ВНЕ корня проекта (его отдаёт backend), поэтому Vite
    // по умолчанию его не чистит и предупреждает об этом. Из-за этого чанки от
    // прежних сборок копились и уезжали в инсталлятор мёртвым грузом — к моменту
    // находки набралось 15 МБ файлов, на которые index.html уже не ссылается.
    emptyOutDir: true,
  },
  server: {
